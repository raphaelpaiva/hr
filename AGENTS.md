...
- Deploy: `./sync.sh` — rsync to `192.168.9.29:~/hr_work/` (excludes venv, git, recordings, sessions)
...
- Sessions persist to `sessions/<id>/session.json` via `src/app/sessions/store.py` (`get_sessions`/`save_session`/`delete_session`), loaded on startup through the app `lifespan` and written atomically (`session.json.tmp` + `os.replace`) on every mutation. Recordings persist to `recordings/<id>/<id>.wav` + `recording.json`; each `recording.json` carries `session_id`/`take_id`.
...
- `static/` is served relative to the **cwd** (`StaticFiles(directory="static")`), recordings go to `./recordings/` and sessions to `./sessions/`. Run the server from the repo root (and the PyInstaller binary from `dist/`).
...
- Tests never touch real hardware or the live `recordings/`/`sessions/` dirs: `tests/conftest.py` swaps in `DummyAlsaSoundSystem`, resets `SESSIONS`, points `BASE_PATH` (in `config`, `recording`, `main`) and `store.SESSIONS_DIR` at a temp dir, and patches `main.run` so `POST /shutdown`'s background `sudo shutdown` never fires. Note this Starlette's `TestClient.delete()` doesn't accept `json=` — use `client.request('DELETE', url, json=...)`.
